...
class Weather():
    ...
    #current - standard
        ...
        set_property('Current.Location'          , '%s, %s' % (data['location']['town']['name'],data['location']['country']['code']))
        set_property('Current.Condition'         , data['location']['conditions']['text'])
        set_property('Current.Temperature'       , convert_temp(data['location']['conditions']['temperature'], 'F', 'C'))
        set_property('Current.UVIndex'           , str(data['conditions']['conditions']['uv']['value']))
        if data['location']['conditions']['text'] in ['Mostly Cloudy', 'Partly Cloudy', 'Fair']:
            condition = '%s Day' % data['location']['conditions']['text']
        else:
            condition = data['location']['conditions']['text']
        ...
        set_property('Current.FeelsLike'         , convert_temp(data['location']['conditions']['realfeel'], 'F', 'C'))
    #current - extended
        set_property('Current.WindChill'         , convert_temp(windchill(data['location']['conditions']['temperature'], data['forecasts'][0]['windForecasts'][0]['speed']), 'F') + TEMPUNIT)
        if 'F' in TEMPUNIT:
    ...
    #forecast - extended
        set_property('Forecast.City'            , data['location']['town']['name'])
        set_property('Forecast.Country'         , data['location']['country']['name'])
        set_property('Forecast.Latitude'        , str(data['location']['town']['latitude']))
        set_property('Forecast.Longitude'       , str(data['location']['town']['longitude']))
        set_property('Forecast.IsFetched'       , 'true')
    #today - extended
        set_property('Today.Sunrise'             , convert_datetime(data['location']['conditions']['sunrise'], 'ampm', None, None))
        set_property('Today.Sunset'              , convert_datetime(data['location']['conditions']['sunset'], 'ampm', None, None))
    ...
    #hourly - extended
        ...
        for count, item in enumerate(data['forecasts'][0]['precipitationForecasts']):
            if count in skip:
                continue
            if 'F' in TEMPUNIT:
                set_property('Hourly.%i.Precipitation'   % (count + 1), str(item['absoluteQuantity']) + 'in')
            else:
                set_property('Hourly.%i.Precipitation'   % (count + 1), str(round(item['absoluteQuantity'] * 25.4)) + 'mm')
            set_property('Hourly.%i.Probability'     % (count + 1), str(item['probabilityOfPrecipitation']) + '%')
